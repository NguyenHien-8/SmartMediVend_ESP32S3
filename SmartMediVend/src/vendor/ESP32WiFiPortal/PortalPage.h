...
<label class="field-label" for="manualPassword"><span>Password</span><span class="password-field"><input class="password-input" id="manualPassword" name="manual-password" type="password" maxlength="64" autocomplete="current-password" placeholder="Leave empty for an open network"><button class="password-reveal" id="manualReveal" type="button" aria-label="Show password" aria-pressed="false" title="Show password"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6S2.5 12 2.5 12Z" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/><circle cx="12" cy="12" r="2.6" fill="none" stroke="currentColor" stroke-width="1.7"/></svg></button></span></label>
...
function credentialError(ssid,password){const s=utf8Length(ssid),p=utf8Length(password);if(s<1||s>32)return'SSID must contain 1-32 bytes.';if(p>64||(p>0&&p<8)||(p===64&&!/^[0-9a-f]{64}$/i.test(password)))return'Password must be empty, contain 8-63 bytes, or be a 64-digit hexadecimal PSK.';return''}
...
function makeNetwork(network){
  ...
  passwordPanel.hidden=true;input.type='password';input.id='ewp-password-'+(++passwordFieldSequence);input.maxLength=64;input.autocomplete='current-password';input.placeholder='Password';label.htmlFor=input.id;reveal.type='button';reveal.title='Hold to show password';reveal.setAttribute('aria-label','Hold to show password');reveal.setAttribute('aria-pressed','false');const eye=svgNode('svg',{viewBox:'0 0 24 24','aria-hidden':'true'});eye.appendChild(svgNode('path',{d:'M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6S2.5 12 2.5 12Z',fill:'none',stroke:'currentColor','stroke-width':'1.7','stroke-linejoin':'round'}));eye.appendChild(svgNode('circle',{cx:'12',cy:'12',r:'2.6',fill:'none',stroke:'currentColor','stroke-width':'1.7'}));reveal.appendChild(eye);passwordField.append(input,reveal);next.type='button';next.disabled=true;cancel.type='button';passwordActions.append(next,cancel);passwordPanel.append(label,passwordField,passwordActions);details.appendChild(passwordPanel);
